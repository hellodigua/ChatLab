/**
 * Database analysis module (server-side)
 * Handles various data analysis queries.
 *
 * Ported from electron/main/database/analysis.ts — no Electron dependencies.
 */
...
/**
 * Build a time-filter WHERE clause.
 */
...
/**
 * Append a system-message exclusion filter.
 */
...
/**
 * Get a member's historical name records.
 */
...
/**
 * Get repeat (echo) analysis data.
 * Uses a sliding window to detect repeat chains:
 * - A chain requires at least 3 consecutive identical messages from different senders
 * - Excludes system messages, empty messages, and image messages
 */
export function getRepeatAnalysis(sessionId: string, filter?: TimeFilter): RepeatAnalysis {
  ...
  try {
    ...
    // Track fastest repeaters: member -> { totalTimeDiff (ms), count }
    const fastestRepeaterStats = new Map<number, { totalTimeDiff: number; count: number }>()

    let currentContent: string | null = null
    let repeatChain: Array<{ senderId: number; content: string; ts: number; id: number }> = []
    let totalRepeatChains = 0
    let totalChainLength = 0

    const processRepeatChain = (
      chain: Array<{ senderId: number; content: string; ts: number; id: number }>,
      breakerId?: number,
    ) => {
      ...
      // Track fastest repeaters (members 1..n-1 who echoed)
      for (let i = 1; i < chain.length; i++) {
        const timeDiff = (chain[i].ts - chain[i - 1].ts) * 1000 // convert to ms
        const mid = chain[i].senderId
        const existing = fastestRepeaterStats.get(mid)
        if (existing) {
          existing.totalTimeDiff += timeDiff
          existing.count++
        } else {
          fastestRepeaterStats.set(mid, { totalTimeDiff: timeDiff, count: 1 })
        }
      }
      ...
      if (existing) {
        ...
        if (chainLength > existing.maxChainLength) {
          ...
          existing.firstMessageId = chain[0].id
        }
      } else {
    ...
    for (const msg of messages) {
      ...
      if (content === currentContent) {
        ...
        if (lastSender !== msg.senderId) {
          repeatChain.push({ senderId: msg.senderId, content, ts: msg.ts, id: msg.id })
        }
      } else {
        processRepeatChain(repeatChain, msg.senderId)

        currentContent = content
        repeatChain = [{ senderId: msg.senderId, content, ts: msg.ts, id: msg.id }]
      }
    }
    ...
    // Build fastest repeater ranking
    const fastestRepeaters: FastestRepeaterItem[] = []
    for (const [memberId, stats] of fastestRepeaterStats.entries()) {
      const info = memberInfo.get(memberId)
      if (info) {
        fastestRepeaters.push({
          memberId,
          platformId: info.platformId,
          name: info.name,
          count: stats.count,
          avgTimeDiff: Math.round(stats.totalTimeDiff / stats.count),
        })
      }
    }
    fastestRepeaters.sort((a, b) => a.avgTimeDiff - b.avgTimeDiff)
...
/**
 * Get weekday activity distribution (Monday–Sunday).
 */
...
/**
 * Get catchphrase analysis data.
 * Counts each member's top-5 most repeated messages.
 * Excludes system messages, empty messages, image messages, and content < 2 chars.
 */
...
/**
 * Get night-owl title based on late-night message count.
 */
...
/**
 * Convert a timestamp to an "adjusted date" (day boundary at 05:00).
 * Messages before 05:00 count toward the previous day.
 */
...
/**
 * Get night-owl analysis data.
 * Late-night window: 23:00–05:00.
 * Day boundary: 05:00 → next 04:59.
 */
...
/**
 * Get dragon king analysis.
 * Counts which member had the most messages each day.
 */
...
/**
 * Get diving (lurker) analysis.
 * Lists all members sorted by how long ago their last message was.
 */
